...
doAllPlot <- function(filename) {
  # setwd("/Users/ablimit/Documents/proj/hadoopgis/gpu/mapred/plots/misc");
  mydata <-read.csv(paste(filename,'.csv', sep=""), header=TRUE);
  m <- ggplot(data=mydata, aes(x=factor(reducer), y=runtime, fill=gpu)) ;
  gplot = m + geom_bar(stat="identity", position=position_dodge()) + xlab("Reducer count") +ylab("Runtime (sec)") ;
