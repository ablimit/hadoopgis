    ...
    void *inserted)			// the being inserted
{
  ...
  entries = (index_entry_t **)malloc((MAX_NR_ENTRIES_PER_NODE + 1) * sizeof(index_entry_t *));
  if(!entries) {
    free(new_node);
    return NULL;
  }
    ...
    r_tree_t *rtree2)
{
  ...
  int nr_poly_pairs = 0;
  int i_poly_pair;
  int size_mbrs, size_idx1, size_idx2;
  ...
  poly_pairs = (poly_pair_array_t *)malloc(sizeof(poly_pair_array_t));
  if(!poly_pairs)
    goto error;
  ...
  size_mbrs = nr_poly_pairs * sizeof(mbr_t);
  size_idx1 = nr_poly_pairs * sizeof(int);
  size_idx2 = nr_poly_pairs * sizeof(int);

  poly_pairs->nr_poly_pairs = nr_poly_pairs;
  poly_pairs->mbrs = (mbr_t *)malloc(size_mbrs + size_idx1 + size_idx2);
  if(!poly_pairs->mbrs)
    goto error;
    ...
    int *nr_poly_pairs)
{
  ...
  if(branch == 0) {
    for(i1 = 0; i1 < node1->nr_entries; i1++) {
      ent1 = (index_entry_t *)&node1->entries[i1];
      for(i2 = 0; i2 < node2->nr_entries; i2++) {
        ent2 = (index_entry_t *)&node2->entries[i2];
    ...
    int *nr_poly_pairs)
{
  ...
  if(node1->node_type == NODE_LEAF) {
    for(i = 0; i < node1->nr_entries; i++) {
      ent = (index_entry_t *)&node1->entries[i];
    ...
    int *nr_poly_pairs)
{
  ...
  if(node2->node_type == NODE_LEAF) {
    for(i = 0; i < node2->nr_entries; i++) {
      ent = (index_entry_t *)&node2->entries[i];
    ...
    int *nr_poly_pairs)
{
  ...
  if(!(*nr_poly_pairs % POLY_PAIR_BLOCK_SIZE)) {
    pblock = (poly_pair_block_t *)malloc(sizeof(poly_pair_block_t));
    if(!pblock)
      return -1;
