    ...
    do
        ...
        do
            ...
            then
                ...
                python genpid.py partres/${f}.${method}.txt < /dev/shm/mbb.txt  > /dev/shm/group.${method}.txt
                python reshardPAIS.py algo${seq}/${f}.markup.ablet.${seq} < /dev/shm/group.${method}.txt > repart/algo${seq}/${f}.${method}.${seq}
            fi
        done
    done
done
