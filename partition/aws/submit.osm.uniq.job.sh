...
do
  echo -n "job param: [${c}] -- "
  elastic-mapreduce --jobflow ${jobid} --stream --step-name "osm.uniq.${algo}.${c}.${log}" --step-action CONTINUE --mapper 's3://aaji/scratch/awsjoin/hgdeduplicator.py cat' --reducer "s3://aaji/scratch/awsjoin/hgdeduplicator.py uniq" --output s3://aaji/scratch/pout/${log}/osm/dedup.${algo}.c${c} --input s3://aaji/scratch/pout/${log}/osm/${algo}c${c} --jobconf mapred.reduce.tasks=1000
