...
udfsinbase=/user/aaji/data/partition/pais
hdfsoutbase=/user/aaji/outputs/pais
...
do
  ...
  for reducecount in 10 20 40 60 80 100 120
  do
    STREAMOPTS="${BASEOPTS} -jobconf mapred.job.name=\"sj_pais_${algo}_${c}_${reducecount}\" -jobconf mapred.task.timeout=36000000"

    START=$(date +%s)
    # join step 
    ${HCMD} -D mapred.child.java.opts=-Xmx4096M -mapper "tagmapper.py pais.geom.1.tsv pais.geom.2.tsv"  -reducer "resque st_intersects 1 1" -file mapper -file reducer -input ${hdfsin} -output ${hdfsout} -numReduceTasks ${reducecount} ${STREAMOPTS}
    END=$(date +%s)
    DIFF=$(( $END - $START ))
