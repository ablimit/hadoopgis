#! /bin/bash

cd

# hdfs dfs -get s3://aaji/scratch/s3cfg .s3cfg

mkdir -p /tmp/lib
#mkdir -p /tmp/include

hdfs dfs -get s3://aaji/scratch/deps/libs/lib* /tmp/lib/
# s3cmd sync s3://aaji/scratch/deps/includes/ /tmp/include/

sudo cp /tmp/lib/* /usr/lib/
#sudo cp -r /tmp/include/* /usr/include/
sudo ln -s /usr/lib/libspatialindex.so.3.0.0 /usr/lib/libspatialindex.so


#s3cmd get s3://aaji/scratch/awsjoin/effresque.cpp ./
#s3cmd get s3://aaji/scratch/awsjoin/makefile ./

#make
# sudo yum install git 


