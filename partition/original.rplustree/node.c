...
/* Initialize a Node structure.
*/
void InitOFNode(struct OverFlowNode *n)
{
    int i;
    n->count = 0;
    n->pageNo = UNKNOWN;
...
*/
void InitBranch(struct Branch *b)
{
    InitRect(&(b->minrect));
    InitRect(&(b->rect));
    b->son = -1;
...
void PrintBranch(struct Branch *b)
{
#               ifdef PRINTINDEX
 ...
 ** branches of a node.
 */
struct Rect NodeCover(struct Node *n)
{
    register int i, flag;
    struct Rect r;
    assert(n);
 ...
 ** branches of a node.
 */
struct Rect MinNodeCover(struct Node *n)
{
    int i, flag;
    struct Rect r;
    int IsOF = FALSE;
 ...
 ** the overflow node.
 */
struct Rect MinOFNodeCover(struct OverFlowNode *overflow, struct Rect rect)
{
    register int i, flag;
    struct Rect r;
 ...
 */
int PickBranch(struct Rect *r, struct Node *n)
{
 ...
 */
int AddBranch(int idxp, struct Branch *b, struct Node *n, struct Node **new)
{
    register int i;
    struct NodeCut cut;
...
*/
void DisconBranch(struct Node *n, int i)
{
...
/* added on 4/22/90 */
int Equal2Rects(struct Rect r1, struct Rect r2)
{
    register int i;
    for(i=0; i<NUMDIMS; i++)
...
*/
int RectInNode(struct Rect r, struct Node *n)
{
    register int i;
 ...
 **
 */
int RectInOFN(struct Rect r, struct OverFlowNode *overflow)
{
    int i;
 ...
 **
 */
void SetOverFlow( struct Rect *r)
{
    int tmp;
