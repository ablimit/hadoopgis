...
typedef map<int,vector<Geometry*> > polymap;
typedef map<int, vector<double*> > cornerMap;
...
class MRJVisitor : public IVisitor
{
	...
	void getInfo(){
		...
		//cerr << "Res: " <<coll[i] <<endl;
		for (int j=0; j< strs.size();j++)
		    indices.push_back(boost::lexical_cast<int>(strs[j]));

		if (polydata[0][indices[0]]->intersects(polydata[1][indices[1]]))
		    c++;
...
RTree::Data* parseInputPolygon(double* inarr, id_type m_id) {
    double low[2], high[2];
...
{
    public:
	MRJDataStream(vector<double*> * invec, int tag ) : m_pNext(0), index(0), len(0),m_id(0)
    {
	if ( invec->empty())
